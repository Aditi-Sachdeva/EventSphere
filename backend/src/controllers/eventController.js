...
const Club = require("./../models/Club");
const mongoose = require("mongoose");
const { sendNewEventEmail } = require("../utils/sendNewEventEmail");
...
async function handleCreateEvent(req, res) {
    try {
        ...
        // Notify all approved club members — fire and forget
        Club.findById(club._id)
            .populate({ path: "members.user", select: "name email" })
            .then(populatedClub => {
                if (!populatedClub) return;
                populatedClub.members
                    .filter(m => m.status === "approved" && m.user)
                    .forEach(m => {
                        sendNewEventEmail(m.user, event, populatedClub)
                            .catch(err => console.error(`📧 Event email failed for ${m.user.email}:`, err.message));
                    });
            });

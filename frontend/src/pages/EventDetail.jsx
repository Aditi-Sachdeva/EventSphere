                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-5 h-full flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="md:hidden flex flex-col justify-center items-center w-9 h-9 rounded-md border border-gray-200 bg-gray-50 hover:bg-gray-100 transition gap-1"
                            aria-label="Toggle menu"
                        >
                            <span className={`block w-4.5 h-0.5 bg-gray-600 transition-all duration-200 ${mobileMenuOpen ? "rotate-45 translate-y-1.5" : ""}`}
                                style={{ width: "18px", height: "2px", display: "block", backgroundColor: "#4B5563", transition: "all 0.2s", transform: mobileMenuOpen ? "rotate(45deg) translate(0, 5px)" : "none" }}
                            />
                            <span className={`block bg-gray-600 transition-all duration-200 ${mobileMenuOpen ? "opacity-0" : ""}`}
                                style={{ width: "18px", height: "2px", display: "block", backgroundColor: "#4B5563", opacity: mobileMenuOpen ? 0 : 1, transition: "all 0.2s" }}
                            />
                            <span className={`block bg-gray-600 transition-all duration-200 ${mobileMenuOpen ? "-rotate-45 -translate-y-1.5" : ""}`}
                                style={{ width: "18px", height: "2px", display: "block", backgroundColor: "#4B5563", transition: "all 0.2s", transform: mobileMenuOpen ? "rotate(-45deg) translate(0, -5px)" : "none" }}
                            />
                        </button>
                    </div>
                ...
                {mobileMenuOpen && (
                    <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-200 shadow-md z-40 px-4 py-3 flex flex-col gap-1" data-mobilemenu>
                        <Link
                            ...
                            className="px-3 py-2 text-sm rounded-md text-gray-700 hover:bg-gray-50 font-medium"
                        >
                            Home
                        </Link>
                        <Link
                            to="/events"
                            onClick={() => setMobileMenuOpen(false)}
                            className="px-3 py-2 text-sm rounded-md bg-gradient-to-r from-pink-500 to-indigo-600 text-white font-semibold"
                        >
                            Events
                        </Link>
                        <Link
            ...
            <div className="pt-16">
                ...
                {loading ? (
                    ...
                    <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-16">
                        ...
                        {/* Hero Banner */}
                        <div className="relative h-48 sm:h-64 md:h-80 bg-gradient-to-br from-pink-400 via-rose-300 to-indigo-400 rounded-2xl overflow-hidden mb-6 sm:mb-8 flex items-center justify-center shadow-xl">
                            {/* Decorative orbs */}
                            ...
                            <span className="text-white font-extrabold opacity-10 select-none hidden sm:block" style={{ fontSize: "12rem", lineHeight: 1 }}>
                                {event.title.charAt(0).toUpperCase()}
                            </span>
                            <span className="text-white font-extrabold opacity-10 select-none sm:hidden" style={{ fontSize: "7rem", lineHeight: 1 }}>
                                {event.title.charAt(0).toUpperCase()}
                            </span>

                            {/* Club badge */}
                            {event.club?.name && (
                                <span className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-white/25 text-white text-xs sm:text-sm font-semibold px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full backdrop-blur-md border border-white/40 shadow-sm max-w-[45%] truncate">
                                    🏛️ {event.club.name}
                                </span>
                            )}
                            ...
                            {/* Status badge */}
                            {isRegistered && (
                                <span className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-green-500/90 text-white text-xs sm:text-sm font-semibold px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full shadow-md backdrop-blur-sm">
                                    ✓ Registered
                                </span>
                            )}
                            {!isRegistered && event.availableSeats === 0 && (
                                <span className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-red-500/90 text-white text-xs sm:text-sm font-semibold px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full shadow-md">
                                    Fully Booked
                                </span>
                            )}

                            {/* Bottom overlay */}
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/30 to-transparent px-4 sm:px-6 py-3 sm:py-4">
                                <p className="text-white/80 text-xs font-medium uppercase tracking-widest truncate">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-8">
                            ...
                            <div className="lg:col-span-3 space-y-5 sm:space-y-6">
                                ...
                                {/* Title + organizer */}
                                <div>
                                    <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-800 mb-3 leading-tight">
                                        {event.title}
                                    </h2>
                                    {event.createdBy?.name && (
                                        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-pink-50 to-indigo-50 border border-pink-100 rounded-full px-3 py-1.5">
                                            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-pink-400 to-indigo-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                    {[
                                        ...
                                        <div
                                            key={label}
                                            className="bg-white border border-gray-100 rounded-xl p-3 sm:p-4 shadow-sm flex items-start gap-3 hover:shadow-md hover:border-pink-100 transition-all duration-200 group"
                                        >
                            ...
                            <div className="lg:col-span-2">
                                <div className="bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden lg:sticky lg:top-24 lg:scale-[1.02]">

                                    {/* Card header strip */}
                                    <div className="bg-gradient-to-r from-pink-400 via-rose-300 to-indigo-400 px-5 sm:px-6 py-4">
                                        ...
                                        {actionStatus === "loading" ? (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <button
                                                    ...
                                                    className="w-full py-3 rounded-full text-sm font-semibold border border-red-200 text-red-500 hover:bg-red-50 hover:border-red-300 transition"
                                                >
                                                    Cancel Registration
                                                </button>
                                            </div>
                                            ...
                                            <button
                                                ...
                                                className="w-full py-3.5 rounded-full text-sm font-semibold bg-gradient-to-r from-pink-500 to-indigo-600 text-white shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all duration-150"
                                            >
                                                Register Now →
                                            </button>
                                        )}

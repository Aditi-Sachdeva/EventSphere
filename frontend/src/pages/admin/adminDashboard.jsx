...
//         axios.get(`${API}/admin/clubs`, auth()),
//         axios.get(`${API}/admin/events`, auth()),
//       ]);
//       setStats({
//         users:  usersRes.data.users.length,
//         clubs:  clubsRes.data.clubs.length,
...
// function Dashboard({ stats, events, clubs }) {
//   const STAT_CARDS = [
//     { label: "Total Users",  value: stats.users,  icon: "👥", bg: "bg-indigo-50", text: "text-indigo-500" },
//     { label: "Total Clubs",  value: stats.clubs,  icon: "🏛", bg: "bg-pink-50",   text: "text-pink-500"   },
//     { label: "Total Events", value: stats.events, icon: "📅", bg: "bg-violet-50", text: "text-violet-600" },
//   ];
...
//         {STAT_CARDS.map(s => (
//           <div key={s.label} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:-translate-y-1 hover:shadow-xl transition-all">
//             <div className={`w-11 h-11 rounded-xl ${s.bg} ${s.text} flex items-center justify-center text-xl mb-4`}>{s.icon}</div>
//             <div className="text-xs text-gray-400 font-medium mb-1">{s.label}</div>
//             <div className="text-3xl font-black text-gray-800 tracking-tight">{s.value}</div>
//           </div>
//         ))}
...
//             <span className="text-sm font-bold text-gray-800">Recent Events</span>
//             {pendingEvents > 0 && (
//               <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200">⚡ {pendingEvents} pending</span>
//             )}
//           </div>
//           {events.length === 0
//             ? <div className="px-4 py-8 text-center text-xs text-gray-400">No events yet</div>
//             : events.map((ev, i) => (
//               <div key={ev._id} className={`flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 transition-colors ${i < events.length - 1 ? "border-b border-gray-50" : ""}`}>
//                 <div className="flex items-center gap-2.5">
//                   <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center">📅</div>
//                   <div>
//                     <div className="text-sm font-semibold text-gray-800">{ev.title}</div>
...
//                   </div>
//                 </div>
//                 <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${ev.status === "approved" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : ev.status === "pending" ? "bg-yellow-50 text-yellow-700 border-yellow-200" : "bg-red-50 text-red-600 border-red-200"}`}>
//                   {ev.status}
//                 </span>
//               </div>
//             ))
...
//                     <div className="flex items-center gap-1.5">
//                       <div className="flex-1 h-1 bg-gray-100 rounded-full overflow-hidden">
//                         <div className={`h-full ${CLUB_COLORS[i % CLUB_COLORS.length]} opacity-80 rounded-full`}
